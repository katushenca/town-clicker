import { overlayState, addMoney } from "./overlay.js";
...
export async function handleClick(bonus = 1) {
  ...
  totalClicks++;
  unsavedClicks++;
  const baseMoneys = Math.round(coinsPerClick + overlayState.population * MONEY_PER_CLICK_POPULATION_FACTOR);
  coinBalance += baseMoneys * extra + (bonus - 1);
  unsavedCoins += baseMoneys * extra + (bonus - 1);
